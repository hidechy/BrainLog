...
?>
            ...
            <tr>
                ...
                @foreach($column as $col)
                    <?php
                    if (!empty($td_bg[$col])){$bgColor = "background : " . $td_bg[$col] . ";";}
                    if (!empty($td_width[$col])){$width = "width : " . $td_width[$col] . "px;";}
					...
					if (in_array($year . "-" . $month . "-" . $line['day'] , $holiday)){$bgColor = "";}
                    ?>
...
<button id="btn_spend_input">input</button>
    ...
    if (count($DisplayKoumoku) > 0){
        ...
        foreach($DisplayKoumoku as $koumoku => $price){
            ...
            $average = '-';
            if ($koumoku == "食費"){$average = number_format(ceil($price / $lastDay)) . "円";}
    ...
    <style type="text/css">
    <!--
    #tbl_money_list td {border : 1px solid #cccccc; text-align : right; }
    #btn_money_input {cursor : pointer; margin : 2px; }
    #btn_money_bank {cursor : pointer; margin : 2px; }
    .btn_prev_next {width : 28px; }
    #btn_money_list {cursor : pointer; margin : 2px; }
    #btn_money_salary {cursor : pointer; margin : 2px; }
    #btn_money_credit {cursor : pointer; margin : 2px; }
    .midashiTd {background : #339933; color : #ffffff; }
    #btn_article_index {cursor : pointer; margin : 2px; }
    #btn_money_repair {cursor : pointer; margin : 2px; }
    #btn_money_history {cursor : pointer; margin : 2px; }

	#tbl_dailyspend td {border : 1px solid #cccccc; text-align : right;}
	.midashi_dailyspend td {background : #3333ff; color : #ffffff; font-weight : bold; text-align : center;}

    #spenddata {width: 600px; height: 200px; font-size: 12px;}

    #KoumokuTable td {border: 1px solid #cccccc;}
    ...
    $("#btn_money_input").click(function (){location.href = "{{ url('/money/input') }}";});

    $("#btn_money_bank").width(38);
    $("#btn_money_bank").click(function (){location.href = "{{ url('/money/bank') }}";});

    $("#btn_money_list").width(38);
    $("#btn_money_list").click(function (){location.href = "{{ url('/money/summary') }}";});

    $("#btn_money_salary").width(38);
    $("#btn_money_salary").click(function (){location.href = "{{ url('/money/salary') }}";});

    $("#btn_money_credit").width(38);
    $("#btn_money_credit").click(function (){location.href = "{{ url('/money/credit') }}";});

    $("#btn_article_index").width(38);
    $("#btn_article_index").click(function (){location.href = "{{ url('/article/index') }}";});

    $("#btn_money_repair").width(38);
    $("#btn_money_repair").click(function (){location.href = "{{ url('/money/repair') }}";});
